		...
		if event.is_action_pressed("interact"):
			...
			if(global.current_area == "area1"):
				...
				if self.get_name() == "SwitchArea1":
					...
					if !global.area1Switch:
						get_tree().get_root().get_child(2).get_node("Sound/MoveBlock").play()
			...
			# House1 switches
				...
				if self.get_name() == "SwitchArea1":
					...
					if global.secretAreaKeyFound:
						...
						get_tree().get_root().get_child(2).get_node("Sound/LockedDoor").play()
			# Secret area switches.
				...
				if self.get_name() == "SwitchArea1":
					# Secret switch behind the grave.
					if !global.secretAreaKeyFound:
						get_tree().get_root().get_child(2).get_node("Sound/PickUp").play()
